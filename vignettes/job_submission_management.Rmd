...
The `Job` class is an R6 class, hence the use of the `$new()` constructor method. `my_job` now contains all the information necessary to submit a job with 8Gb and a simple "Hello world" bash command.
...
You can change the folder where these files will be generated through the `outDir` argument of `$new()` constructor method.
